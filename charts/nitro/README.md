...
| `eventFilter.enabled`                                       | Render an event-filter ConfigMap and wire it into the pod. Also auto-injects `execution.transaction-filtering.event-filter.path`; listing that key in `configmap.removeKeys` will strip the path from the rendered config even though the volume is still mounted. | `false`                                                             |
| `eventFilter.rules`                                         | Inline rules list, serialized to JSON as `{"rules": [...]}` and written to the ConfigMap. Schema is defined by Nitro.                                                                                                                                              | `[]`                                                                |
| `eventFilter.mountPath`                                     | Path inside the container where the rules file is mounted                                                                                                                                                                                                          | `/etc/nitro/event-filter`                                           |
| `eventFilter.fileName`                                      | File name of the rules JSON inside the mount                                                                                                                                                                                                                       | `event-filter.json`                                                 |
...
Nitro can filter incoming transactions by event signature using a rules file referenced by `execution.transaction-filtering.event-filter.path`. The chart can render the ConfigMap, mount it into the pod, and wire the config path for you — set `eventFilter.enabled: true` and supply the rules under `eventFilter.rules`. The ConfigMap (named `<release>-event-filter`), the volume, the mount, and the config path are all derived from the same values, so there is nothing to keep in sync by hand. See the [Arbitrum documentation](https://docs.arbitrum.io/) for the rules schema.

Before (manual pattern with a sibling ConfigMap and `extraVolumes`/`extraVolumeMounts`):
...
nitro:
  extraVolumes:
    - name: event-filters
      configMap:
        name: my-release-event-filters
  extraVolumeMounts:
    - mountPath: /event-filters
      name: event-filters
      readOnly: true
  configmap:
    data:
      execution:
        transaction-filtering:
          event-filter:
            path: /event-filters/event-filter.json
# Plus a separately-managed ConfigMap named "my-release-event-filters"
# created by another chart or kubectl apply.
```
