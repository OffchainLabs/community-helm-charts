...
| `validator.splitvalidator.global.stsAffinity.zoneTopologyKey`                       | Topology key for the zone-level same-zone rule. Default is the standard Kubernetes well-known label, which is populated by AWS, GCP, Azure, and other CNCF-conformant providers. Override for vendor-specific labels; set to empty string to disable zone affinity.                                                                                                                                                            | `topology.kubernetes.io/zone` |
| `validator.splitvalidator.global.stsAffinity.nodeTopologyKey`                       | Topology key for the node-level same-node preference. Default is the standard Kubernetes well-known label. Override for vendor-specific labels; set to empty string to disable node affinity.                                                                                                                                                                                                                                  | `kubernetes.io/hostname`      |
