...
| `env.nitro.goMemLimit.nonGoMemory.enabled`                  | Enable non-Go memory subtraction from GOMEMLIMIT calculation                              | `true`                                                              |
| `env.nitro.goMemLimit.nonGoMemory.defaults.databaseCache`   | Default Pebble block cache size in MiB (CGO)                                              | `2048`                                                              |
| `env.nitro.goMemLimit.nonGoMemory.defaults.trieCleanCache`  | Default trie-clean-cache size in MiB (fastcache/mmap)                                     | `600`                                                               |
| `env.nitro.goMemLimit.nonGoMemory.defaults.snapshotCache`   | Default snapshot-cache size in MiB (fastcache/mmap)                                       | `400`                                                               |
| `env.nitro.goMemLimit.nonGoMemory.defaults.stylusLruCache`  | Default stylus-lru-cache size in MiB (Rust/jemalloc)                                      | `256`                                                               |
| `env.nitro.goMemLimit.nonGoMemory.defaults.mallocArenaSize` | Default glibc malloc arena size in MiB                                                    | `64`                                                                |
| `env.nitro.mallocArenaMax.enabled`                          | Enable setting MALLOC_ARENA_MAX to limit glibc arena count                                | `true`                                                              |
| `env.nitro.mallocArenaMax.value`                            | Value for MALLOC_ARENA_MAX                                                                | `2`                                                                 |
...
| `resources.requests.ephemeral-storage`                      | Ephemeral storage request for the container                                               | `100Mi`                                                             |
...
| `validator.splitvalidator.global.stsAffinity.enabled`                               | Co-locate validator pods with the nitro statefulset pods. The statefulset is the anchor (its PVC pins it to one zone); validator pods follow it to reduce cross-AZ traffic and latency.                                                                                                                                                                                                                                        | `false`                       |
| `validator.splitvalidator.global.stsAffinity.provider`                              | Cloud provider preset for topology keys. Built-in: kubernetes, aws, gcp, azure. All built-ins resolve to the standard Kubernetes well-known labels, which are populated by all major cloud providers. Use 'custom' with `topologyKeys` for vendor-specific labels.                                                                                                                                                             | `kubernetes`                  |
| `validator.splitvalidator.global.stsAffinity.topologyKeys.zone`                     | Direct override for the zone topology key. Takes precedence over the provider preset. Empty string uses the provider default; set to "-" to disable zone affinity.                                                                                                                                                                                                                                                             | `""`                          |
| `validator.splitvalidator.global.stsAffinity.topologyKeys.node`                     | Direct override for the node topology key. Takes precedence over the provider preset. Empty string uses the provider default; set to "-" to disable node affinity.                                                                                                                                                                                                                                                             | `""`                          |
| `validator.splitvalidator.global.stsAffinity.zoneMode`                              | 'preferred' (default) favors same-zone scheduling but allows fallback to other zones if the preferred zone is unavailable. 'required' enforces same-zone strictly — validator pods will stay Pending on first install until the statefulset pod is scheduled (Helm creates Deployments before StatefulSets), and on disruption events validator pods may remain Pending if no capacity is available in the statefulset's zone. | `preferred`                   |
| `validator.splitvalidator.global.stsAffinity.weight`                                | Weight (1-100) for preferred affinity rules (same-node preference, and same-zone preference when zoneMode is 'preferred').                                                                                                                                                                                                                                                                                                     | `100`                         |
...
`blocks-reexecutor.room` | int                                                                             number of threads to parallelize blocks re-execution | `2`
...
`init.prune-threads` | int                                                                                 the number of threads to use when pruning | `2`
...
`node.block-validator.prerecorded-blocks` | uint                                                           record that many blocks ahead of validation (larger footprint) | `4`
...
`persistent.pebble.max-concurrent-compactions` | int                                                       maximum number of concurrent compactions | `2`
