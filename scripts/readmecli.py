...
def render_helm_chart(chart_path, values_path):
    ...
    try:
        ...
        logging.info(f"Rendering Helm chart: {chart_path}")
...
def format_cli_help_to_markdown(cli_output):
    ...
    for line in lines:
        ...
        if command_match:
            ...
            default_match = re.search(r'\(default\s+["\']?(.+?)["\']?\)$', rest_of_line)
            if default_match:
                current_default = default_match.group(1)
                current_description[-1] = (
                    current_description[-1].replace(default_match.group(0), "").strip()
                )
...
def main():
    ...
    for directory in list_directories(charts_dir):
        ...
        if chart_yaml and values_yaml:
            logging.info(f"Processing chart: {chart_path}")
            ...
            logging.info(f"Image: {image_repository}:{image_tag}")
            ...
            if image_repository and app_version:
                ...
                if rendered_templates:
                    ...
                    if help_output:
                        ...
                        update_readme(
                            os.path.join(chart_path, "README.md"), config_data
                        )
                    else:
                        logging.warning(
                    ...
                    logging.warning(f"Failed to render Helm chart: {chart_path}")
